import {INFO_AUTHOR, LIST_ITEM_NAVBAR, SIZE_ICON} from "@/constant";
...
import {IValidField} from "@/constant/type";
...
export function Contact(props: Props) {
    ...
    const handleSubmit = async (e: React.SyntheticEvent) => {
        ...
        try {
            ...
            const response = await fetch("https://formsubmit.co/ajax/garu097test@gmail.com", {
                method: 'POST',
                body: formData
            });
            if (response?.ok) {
                ...
                <Fade>
                    <div className={styles.form_container}>
                        ...
                        <form onSubmit={handleSubmit} className={styles.form_content}>
                            <div className={clsx(styles.group_input, styles.group_input_name)}>
                                <input name="name"
                                       ...
                                       className={clsx(styles.form_input, {[`${styles.input_active}`]: fieldForm.name})}
                            ...
                            <div className={clsx(styles.group_input, styles.group_input_email)}>
                                <input name="email"
                                       ...
                                       className={clsx(styles.form_input, {[`${styles.input_active}`]: fieldForm.email})}
                        ...
                        <textarea
                            ...
                            name="message"
                            rows={4}
                            className={clsx(styles.form_textarea, {[`${styles.input_active}`]: fieldForm.message})}
                            ...
                            {isSendMessage && <Fade className={styles.message_sent}>
                                <div>The message has been sent!
                                    Thank you!
                                </div>
                            </Fade>}
                        </form>
                    ...
                    <div className={styles.socials}>
                        ...
                        <div className={styles.group_socials}>
                            {Object.keys(INFO_AUTHOR).map(key =>
                                <a key={key} href={INFO_AUTHOR[key]?.url} target="_blank"
                                   className={styles.link_icon}
                                   rel="noreferrer">
                                    <Image src={INFO_AUTHOR[key].icon} width={SIZE_ICON.DEFAULT.width}
